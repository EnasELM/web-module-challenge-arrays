...
function copy(originalFlavorscopy){
  for(let i =0; i<originalFlavors.length;i++){
    originalFlavorscopy[i]=originalFlavors[i];
  }
...
function is31Flavors(originalFlavorscheck){
  if (originalFlavorscheck.length=31){
    return( true);
  }
  else {
    return(false);
...
function addFlavor(addFlavors){
 addFlavors.unshift("Rainbow Sherbert");
 return addFlavors;
...
function removeLastFlavor(removeFlavor){
...
function getFlavorByIndex(FlavorByIndex,index){
  let favorite = FlavorByIndex[index];
  return favorite;
}
...
*/
function removeFlavorByName(removeflavor,string){
  for (let i=0;i<=removeflavor.length;i++){
    if (removeflavor[i]===string){
      removeflavor.splice(i, 1);
      return removeflavor;
      break;
